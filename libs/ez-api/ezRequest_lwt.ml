...
module Make(S : sig
  ...
  let get0 ?(post=false) ?headers ?(params=[])
      api (service:'output EzAPI.service0) msg =
    !before_hook ();
  ...
  let get1 ?(post=false) ?headers ?(params=[])
      api (service : ('arg,'output) EzAPI.service1) msg (arg : 'arg) =
    !before_hook ();
    let encoding = EzAPI.service_output service in
    if post then
  ...
  let post0 ?headers ?(params=[]) ~(input : 'input)
      api ( service : ('input,'output) EzAPI.post_service0 ) msg =
    !before_hook ();
  ...
  let post1 ?headers ?(params=[]) ~(input : 'input)
      api (service : ('arg, 'input,'output) EzAPI.post_service1 ) msg (arg : 'arg) =
    !before_hook ();
    let input_encoding = EzAPI.service_input service in
    let output_encoding = EzAPI.service_output service in
    let url = EzAPI.forge1 api service arg params in
    let content = EzEncoding.construct input_encoding input in
    let content_type = "application/json" in
    internal_post msg url ~content ~content_type ?headers >|=
    (decode_result output_encoding)
