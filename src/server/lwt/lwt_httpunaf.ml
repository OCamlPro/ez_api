...
let create_server ?(name="HTTPUN") server_port server_kind handler =
  ...
  EzDebug.printf "Starting %s server (port=%d)" name server_port;
...
let server ?name ?catch ?allow_origin ?footer handler servers =
  ...
  Lwt.join (List.map (fun (port, kind) ->
      ...
      create_server ?name port kind handler) servers) >>= fun () ->
  waiter
