...
let date_of_tm ?(sep="T") ?(fin="Z") tm =
  Format.sprintf "%04d-%02d-%02d%s%02d:%02d:%02d%s"
...
let pp ?(sep=" ") ?(fin="") fmt tm =
  Format.fprintf fmt "%s" (date_of_tm ~sep ~fin tm)

let pp_now fmt =
  pp fmt (Unix.gmtime (Unix.gettimeofday ()))
