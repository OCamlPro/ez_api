...
module TYPES = struct
  ...
  module type SessionArg = sig
    ...
    (** Web host, that should be used in access control headers, if specified. If web_host isn't specified,
        ...
        Last one in turn, requires that "access-control-allow-origin" header by reponse returns something
        different from "*". *)
...
module Make(S : SessionArg) = struct
  ...
  module Service = struct
    ...
    *)
    ...
    let headers =
      EzAPI.StringMap.singleton "access-control-allow-origin"
        (EzAPI.StringSet.singleton (match S.web_host with None -> "*" | Some origin -> origin))
